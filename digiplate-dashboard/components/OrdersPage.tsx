...
import { collection, onSnapshot, doc, updateDoc, query, where, orderBy, Timestamp, writeBatch, getDocs, increment, getDoc } from 'firebase/firestore';
...
const OrdersPage: React.FC<OrdersPageProps> = ({ userId, onNewOrder, profile, stores, role }) => {
    ...
    const handleMarkAsComplete = async (orderId: string) => {
        ...
        // 3. Update floor plan table status
    ...
    const handleUpdateItemStatus = async (orderId: string, itemIndex: number, newStatus: boolean) => {
        const orderRef = doc(db, 'orders', orderId);
        const orderToUpdate = activeOrders.find(o => o.id === orderId) || completedOrders.find(o => o.id === orderId);
