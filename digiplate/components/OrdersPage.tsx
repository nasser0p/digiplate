...
import { collection, onSnapshot, doc, updateDoc, query, where, orderBy, Timestamp, writeBatch, getDocs, increment } from 'firebase/firestore';
import { db } from '../firebase';
import { Order, OrderStatus, Store, PrepItem, RestaurantProfile, Role, MenuItem } from '../types';
...
const OrdersPage: React.FC<OrdersPageProps> = ({ userId, onNewOrder, profile, stores, role }) => {
    ...
    // Fetch all menu items once for recipe lookups
     useEffect(() => {
        if (!userId) return;
    ...
    useEffect(() => {
        ...
        const activeStatuses: OrderStatus[] = ['New', 'In Progress', 'Ready'];
        const q = query(
        ...
        const unsubscribeActive = onSnapshot(q, (snapshot) => {
            ...
            const currentNewOrderIds = new Set(newOrders.map(o => o.id));
            if (previousNewOrderIdsRef.current.size > 0) { 
                currentNewOrderIds.forEach(id => {
                    if (!previousNewOrderIdsRef.current.has(id)) {
                        onNewOrder();
                    }
                });
            }
    ...
    const filteredActiveOrders = useMemo(() => {
        return activeOrders.filter(order => {
            const matchesStore = filterStoreId === 'all' || order.storeId === filterStoreId;
            const matchesSearch = !searchQuery || order.plateNumber?.toLowerCase().includes(searchQuery.toLowerCase());
            return matchesStore && matchesSearch;
        });
    }, [activeOrders, searchQuery, filterStoreId]);

    const ordersByStatus = useMemo(() => {
        const result: Record<OrderStatus, Order[]> = { 'New': [], 'In Progress': [], 'Ready': [], 'Completed': [] };
    ...
    const prepItems = useMemo<PrepItem[]>(() => {
        ...
        const itemMap = new Map<string, { totalQuantity: number; orders: { orderId: string; plateNumber?: string; quantity: number }[] }>();
    ...
    const handleMarkAsComplete = async (orderId: string) => {
        ...
        deductions.forEach((quantityToDeduct, ingredientId) => {
            ...
            batch.update(ingredientRef, { stock: increment(-quantityToDeduct) });
        });
    ...
    const statusConfig: Record<OrderStatus, { title: string; color: string; }> = {
        'New': { title: t('orders_page_status_new'), color: 'bg-blue-500' },
        'In Progress': { title: t('orders_page_status_in_progress'), color: 'bg-yellow-500' },
        'Ready': { title: t('orders_page_status_ready'), color: 'bg-green-500' },
        'Completed': { title: t('orders_page_status_completed'), color: 'bg-gray-500' },
    };
    ...
    const renderKanbanView = () => (
        <DragDropContext onDragEnd={onDragEnd}>
            <div data-tour-id="orders-kanban" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 h-full min-h-[500px]">
                {(['New', 'In Progress', 'Ready'] as OrderStatus[]).map(status => (
                    <div key={status} className="bg-brand-gray-100 dark:bg-brand-gray-800 rounded-xl flex flex-col h-full">
                        <div className={`p-4 rounded-t-xl ${statusConfig[status].color}`}>
                            <h3 className="font-bold text-white text-lg">{statusConfig[status].title} ({ordersByStatus[status].length})</h3>
                        </div>
                        <Droppable droppableId={status} isDropDisabled={role === 'kitchen_staff'}>
                            {(provided, snapshot) => (
                                <div
             ...
             <div className="bg-white dark:bg-brand-gray-900 p-4 rounded-xl shadow-md flex flex-wrap items-center justify-between gap-4">
                <div className="flex items-center gap-4 flex-wrap">
                    <input
                        ...
                        className="px-3 py-2 w-48 bg-white dark:bg-brand-gray-700 border border-brand-gray-300 dark:border-brand-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-brand-teal focus:border-brand-teal"
                    />
                    <select
                        value={filterStoreId}
                        onChange={(e) => setFilterStoreId(e.target.value)}
                        className="px-3 py-2 bg-white dark:bg-brand-gray-700 border border-brand-gray-300 dark:border-brand-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-brand-teal focus:border-brand-teal"
                    >
