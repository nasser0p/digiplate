...
const OrderCard: React.FC<OrderCardProps> = ({ order, index, onMarkComplete, onToggleUrgent, onCardClick, onPrint, role }) => {
            ...
            {item.selectedModifiers && item.selectedModifiers.length > 0 && (
                <div className="ps-4 text-xs text-brand-gray-500">
                    {item.selectedModifiers.map((mod, modIndex) => (
                        <div key={modIndex}>&bull; {mod.groupName}: {mod.optionName}</div>
                    ))}
                </div>
            )}
        </li>
                    ...
                    <div className="p-4">
                        <div className="flex justify-between items-start mb-2">
                            <div>
                                ...
                                {order.plateNumber && <p className="text-sm font-semibold text-brand-teal">{t('order_card_plate')} {order.plateNumber}</p>}
                            ...
                            <div className="flex items-center gap-3">
                                 <button
                                    ...
                                    className="p-1.5 rounded-full bg-brand-gray-200 dark:bg-brand-gray-700 text-brand-gray-600 dark:text-brand-gray-300 hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors"
                                >
                                ...
                                {canMarkUrgent && (
                                 ...
                                 <div className="text-center flex-shrink-0">
                                    <span className="font-mono text-xl font-bold text-brand-gray-700 dark:text-brand-gray-200">{timer}</span>
                                    <p className="text-xs text-brand-gray-500 dark:text-brand-gray-400">{t('order_card_time_active')}</p>
                                </div>
                            </div>
                        </div>
