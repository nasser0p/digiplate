            ...
            case 'grid':
                return (
                    <div className="grid grid-cols-1 gap-4 p-4">
                        {items.map(item => <CustomerItemCard key={item.id} item={item} onSelectItem={() => {}} promotion={null} />)}
                    </div>
                );
            case 'list':
            case 'elegant':
                ...
                return (
                    <div className={`space-y-3 p-4 ${layoutClasses}`}>
                        {items.map(item => <CustomerItemRow key={item.id} item={item} onSelectItem={() => {}} promotion={null} />)}
                    </div>
                );
        }
    }
        ...
        <div className={`w-full h-full overflow-y-auto ${fontClass} ${mainClasses}`} style={style}>
            <header className="relative">
                {appearance.headerBannerUrl && (
                    <img src={appearance.headerBannerUrl} alt="Banner" className="w-full h-32 object-cover"/>
                )}
                <div className={`p-4 ${appearance.headerBannerUrl ? 'absolute bottom-0 left-0 w-full bg-gradient-to-t from-black/60 to-transparent' : ''}`}>
                     <div className="flex items-center space-x-3">
                        {profile?.logoUrl && <img src={profile.logoUrl} alt="Logo" className="h-12 w-12 object-cover rounded-full shadow-md" />}
                        <div>
                            <h1 className={`text-2xl font-bold ${appearance.headerBannerUrl ? 'text-white drop-shadow-md' : 'customer-text'}`}>{profile?.name}</h1>
                        </div>
                    </div>
                </div>
            </header>
