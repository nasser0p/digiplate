...
import { collection, Timestamp, query, where, doc, getDoc, getDocs, addDoc, orderBy, setDoc } from 'firebase/firestore';
import { db } from '../../firebase';
import { MenuItem, CartItem, Store, RestaurantProfile, OrderItem, Category, MenuAppearance, Promotion, AppliedDiscount, SpecialOffer, MultiBuyOffer } from '../../types';
...
const CustomerMenuPage: React.FC<CustomerMenuPageProps> = ({ storeId, restaurantId, tableNumber }) => {
    ...
    if (view === 'checkout') {
        return <CheckoutView 
            ...
            customerPhoneNumber={customerPhoneNumber} onCustomerPhoneNumberChange={setCustomerPhoneNumber}
        />
    }
    
    const fontClass = `font-${appearance.fontTheme}`;
    const mainClasses = `customer-bg customer-text`;
        ...
        <div className={`${mainClasses} min-h-screen ${fontClass}`}>
             <header className="relative">
                {appearance.headerBannerUrl && (
                    <img src={appearance.headerBannerUrl} alt="Banner" className="w-full h-48 object-cover"/>
                )}
                <div className={`p-4 ${appearance.headerBannerUrl ? 'absolute bottom-0 left-0 w-full bg-gradient-to-t from-black/70 to-transparent' : 'bg-white/80 dark:bg-brand-gray-900/80 backdrop-blur-md shadow-sm'}`}>
                    <div className={`${layoutWrapperClasses} flex items-center justify-between space-x-4 rtl:space-x-reverse`}>
                        <div className="flex items-center space-x-4">
                            ...
                            <div>
                                <h1 className={`text-2xl font-bold ${appearance.headerBannerUrl ? 'text-white drop-shadow-md' : 'customer-text'}`}>{profile?.name || t('customer_menu_welcome')}</h1>
                                <p className={`text-sm font-medium ${appearance.headerBannerUrl ? 'text-gray-200 drop-shadow' : 'text-brand-gray-500 dark:text-brand-gray-400'}`}>{profile?.address}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                {showOnlyPromotions ? (
                     <section className="mb-8 pt-4">
                        ...
                        {appearance.layout === 'grid' ? (
                             <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                                {promotedItems.map(item => <CustomerItemCard key={item.id} item={item} onSelectItem={handleSelectItem} promotion={itemPromotions.get(item.id) || null} />)}
                            </div>
                        ) : (
                            <div className="space-y-4">
                               {promotedItems.map(item => <CustomerItemRow key={item.id} item={item} onSelectItem={handleSelectItem} promotion={itemPromotions.get(item.id) || null} />)}
                            </div>
                        )}
                    </section>
                ) : (
                    categories.map(category => (
                        <section key={category.id} id={category.id} ref={el => { categoryRefs.current[category.id] = el; }} className="mb-8 pt-4 scroll-mt-32">
                            ...
                            {appearance.layout === 'grid' ? (
                                 <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                                    {getItemsForCategory(category.name).map(item => <CustomerItemCard key={item.id} item={item} onSelectItem={handleSelectItem} promotion={itemPromotions.get(item.id) || null} />)}
                                </div>
                            ) : (
                                <div className="space-y-4">
                                   {getItemsForCategory(category.name).map(item => <CustomerItemRow key={item.id} item={item} onSelectItem={handleSelectItem} promotion={itemPromotions.get(item.id) || null} />)}
                                </div>
                            )}
                        </section>
                    ))
